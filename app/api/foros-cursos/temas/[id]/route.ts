...
/**
 * Edición de un tema propio (Módulo 8, Paso 3) — bloqueada si el tema ya
 * fue moderado por un Administrador, verificado en
 * `EditarTemaForo.autorizar()` (PEA-SIS-002 / PEA-FORO-004), nunca acá.
 */
export async function PATCH(request: NextRequest, { params }: ContextoRuta) {
  ...
  try {
    const casoDeUso = container.resolve(EditarTemaForo);
    const resultado = await casoDeUso.ejecutar({ datosCrudos: cuerpo, temaId: params.id, usuarioId: usuarioAutenticado.id });
    ...
    logger.error({ err: error }, 'Error no controlado en PATCH /api/foros-cursos/temas/[id]');
