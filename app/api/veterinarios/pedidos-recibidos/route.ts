...
/** Pedidos recibidos sobre productos propios (`veterinario RU(p, de sus productos)`, docs/ROLES.md Módulo 6). */
export async function GET(request: NextRequest) {
  ...
  const { pagina, porPagina } = ListarProductosQuerySchema.parse(
    Object.fromEntries(request.nextUrl.searchParams),
  );

  try {
    const casoDeUso = container.resolve(ListarPedidosRecibidos);
    ...
    logger.error({ err: error }, 'Error no controlado en GET /api/veterinarios/pedidos-recibidos');
