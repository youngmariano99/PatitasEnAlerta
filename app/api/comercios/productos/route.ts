...
/**
 * Alta de un producto del catálogo propio (Módulo 7, Paso 1) — exclusivo de
 * rol `comerciante` con comercio verificado, verificado en
 * `PublicarProductoComercio.autorizar()` (PEA-SIS-002 / PEA-COM-001 /
 * PEA-COM-003), nunca acá.
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    logger.error({ err: error }, 'Error no controlado en POST /api/comercios/productos');
    return respuestaDeError(
      'PEA-SIS-003',
      'Algo salió mal de nuestro lado. Ya estamos al tanto, probá de nuevo en unos minutos.',
      500,
