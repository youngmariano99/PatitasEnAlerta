...
function crearFakes(opciones?: {
  ...
  const repositorioTurnos: jest.Mocked<IRepositorioTurnos> = {
    ...
    obtenerActual: jest.fn().mockImplementation(async (id: string) => {
      if (id === turnoActualId)
        return opciones?.actual === undefined ? actualReservado : opciones.actual;
      if (id === turnoNuevoId)
        return opciones?.nuevo === undefined ? nuevoDisponible : opciones.nuevo;
      return null;
    }),
    reprogramar: jest
      .fn()
      .mockResolvedValue(
        opciones?.reprogramarDevuelve === undefined
          ? resultadoPorDefecto
          : opciones.reprogramarDevuelve,
      ),
...
describe('ReprogramarTurnoCommand', () => {
  it('AC (Paso 2): cancela el turno actual y reserva el nuevo dentro de la misma transacción (delegado a IRepositorioTurnos.reprogramar)', async () => {
    ...
    const resultado = await caso.ejecutar({
      datosCrudos: { turnoActualId, turnoNuevoId },
      usuarioId: reservante,
    });

    expect(repositorioTurnos.reprogramar).toHaveBeenCalledWith(
      turnoActualId,
      turnoNuevoId,
      reservante,
      5,
  ...
  it('rechaza (Zod) cuando el turno nuevo es el mismo que el actual, sin tocar el repositorio', async () => {
    ...
    await expect(
      caso.ejecutar({
        datosCrudos: { turnoActualId, turnoNuevoId: turnoActualId },
        usuarioId: reservante,
      }),
    ).rejects.toThrow();
  ...
  it('404 / PEA-MUN-003 cuando el turno actual no está reservado (ya cancelado)', async () => {
    const { repositorioTurnos } = crearFakes({
      actual: { ...actualReservado, estado: 'cancelado' },
    });
    const caso = new ReprogramarTurnoCommand(repositorioTurnos);
