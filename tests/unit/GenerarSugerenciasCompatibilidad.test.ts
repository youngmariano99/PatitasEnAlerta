...
import {
  CuestionarioIncompletoError,
  CuestionarioNoEncontradoError,
} from '@dominio/errores/erroresAdopcionCompatibilidad';
...
function crearFakes(opciones?: {
  cuestionario?: CuestionarioAdoptante | null;
  candidatos?: FichaAdopcion[];
}) {
  const candidatos = opciones?.candidatos ?? [crearFicha('ficha-1')];
  const pagina: PaginaFichasAdopcion = {
    items: candidatos,
  ...
  const repositorioCuestionarios: jest.Mocked<IRepositorioCuestionariosAdoptante> = {
    obtenerPropio: jest
      .fn()
      .mockResolvedValue(
        opciones && 'cuestionario' in opciones ? opciones.cuestionario : cuestionarioCompleto,
      ),
    crear: jest.fn(),
    actualizar: jest.fn(),
  };
...
describe('GenerarSugerenciasCompatibilidad', () => {
  ...
  it('Verificación técnica: nunca invoca nada distinto de la estrategia inyectada', async () => {
    ...
    expect(fakes.estrategiaCompatibilidad.calcularScore).toHaveBeenCalledWith(
  ...
  it('responde 404 / PEA-ADOP-003 si el usuario nunca completó un cuestionario', async () => {
    ...
    await expect(caso.ejecutar({ usuarioId })).rejects.toBeInstanceOf(
      CuestionarioNoEncontradoError,
    );
  ...
  it('responde 400 / PEA-ADOP-001 si el cuestionario propio está incompleto', async () => {
    const fakes = crearFakes({
      cuestionario: { ...cuestionarioCompleto, experienciaPrevia: null },
    });
    const caso = new GenerarSugerenciasCompatibilidad(
      fakes.repositorioCuestionarios,
      fakes.repositorioFichas,
