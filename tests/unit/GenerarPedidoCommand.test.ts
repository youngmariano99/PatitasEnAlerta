...
} from '@dominio/errores/erroresVeterinariosAvanzados';
...
function crearFakes(opciones?: {
  ...
  const repositorioPedidos: jest.Mocked<IRepositorioPedidosProducto> = {
    crear: jest
      .fn()
      .mockResolvedValue(
        opciones && 'pedidoCreado' in opciones ? opciones.pedidoCreado : pedidoPorDefecto,
      ),
...
describe('GenerarPedidoCommand', () => {
  it('Paso 2: genera el pedido cuando hay stock suficiente y quien compra es dueño', async () => {
    const { repositorioProductos, repositorioPedidos, repositorioPerfil, pedidoPorDefecto } =
      crearFakes();
    const caso = new GenerarPedidoCommand(
  ...
  it.each([
    ...
    'administrador',
    'comerciante',
  ])('rechaza con 403 / PEA-SIS-002 a un usuario con rol %s', async (rol) => {
    ...
    await expect(
      caso.ejecutar({ datosCrudos: { cantidad: 1 }, productoId, compradorId }),
  ...
  it('rechaza con 404 / PEA-VETADV-002 si el producto no existe o está soft-deleted', async () => {
    ...
    await expect(
      caso.ejecutar({ datosCrudos: { cantidad: 1 }, productoId, compradorId }),
    ).rejects.toBeInstanceOf(ProductoNoDisponibleError);
    expect(repositorioPedidos.crear).not.toHaveBeenCalled();
  });
  ...
  it('AC: rechaza con 409 / PEA-VETADV-001 si no queda stock suficiente', async () => {
    ...
    await expect(
      caso.ejecutar({ datosCrudos: { cantidad: 100 }, productoId, compradorId }),
  ...
  it('rechaza cantidad cero o negativa (400, fail-fast vía Zod)', async () => {
    ...
    await expect(
      caso.ejecutar({ datosCrudos: { cantidad: 0 }, productoId, compradorId }),
    ).rejects.toThrow();
    expect(repositorioPedidos.crear).not.toHaveBeenCalled();
  });
});
