...
function crearPerfil(id: string, rol: string): ResumenPerfilPropio {
  return { id, email: 'vet@ejemplo.test', rol, estadoVerificacion: 'verificado', verificadoEn: new Date() };
}
...
function crearFakes(opciones?: { rolOrigen?: string; rolDestino?: string; mascota?: Mascota | null }) {
  ...
  const repositorioMascotas: jest.Mocked<IRepositorioMascotas> = {
    crear: jest.fn(),
    buscarPorId: jest.fn().mockResolvedValue(opciones?.mascota === undefined ? crearMascota() : opciones.mascota),
  ...
  const repositorioPerfil: jest.Mocked<IRepositorioPerfil> = {
    obtenerPerfilPropio: jest.fn(async (id: string) => {
      ...
      if (id === veterinarioDestinoId) return crearPerfil(id, opciones?.rolDestino ?? 'veterinario');
      return null;
    }),
  };
...
describe('CompartirHistorial', () => {
  it('Paso 1: crea el historial compartido con veterinarioOrigenId de la sesión', async () => {
    const { repositorioHistoriales, repositorioMascotas, repositorioPerfil, historialCreado } = crearFakes();
    const caso = new CompartirHistorial(repositorioHistoriales, repositorioMascotas, repositorioPerfil);
    ...
    expect(repositorioHistoriales.crear).toHaveBeenCalledWith({ mascotaId, veterinarioOrigenId, veterinarioDestinoId });
  });
  ...
  it('AC: rechaza con 400 / PEA-VETADV-003 cuando origen y destino son el mismo veterinario', async () => {
    ...
    await expect(
      caso.ejecutar({ datosCrudos: { mascotaId, veterinarioDestinoId: veterinarioOrigenId }, veterinarioOrigenId }),
  ...
  it.each(['dueño', 'rescatista', 'organizacion', 'municipio', 'administrador', 'comerciante'])(
    ...
    async (rol) => {
      const { repositorioHistoriales, repositorioMascotas, repositorioPerfil } = crearFakes({ rolOrigen: rol });
      const caso = new CompartirHistorial(repositorioHistoriales, repositorioMascotas, repositorioPerfil);

      await expect(caso.ejecutar({ datosCrudos: datosValidos, veterinarioOrigenId })).rejects.toBeInstanceOf(AccesoNoAutorizadoError);
      expect(repositorioHistoriales.crear).not.toHaveBeenCalled();
    },
  );

  it('responde 404 / PEA-AUTH-009 si la mascota no existe', async () => {
    const { repositorioHistoriales, repositorioMascotas, repositorioPerfil } = crearFakes({ mascota: null });
    const caso = new CompartirHistorial(repositorioHistoriales, repositorioMascotas, repositorioPerfil);

    await expect(caso.ejecutar({ datosCrudos: datosValidos, veterinarioOrigenId })).rejects.toBeInstanceOf(MascotaNoEncontradaError);
    expect(repositorioHistoriales.crear).not.toHaveBeenCalled();
  });

  it('responde 404 / PEA-VET-011 si el destino no corresponde a un veterinario', async () => {
    const { repositorioHistoriales, repositorioMascotas, repositorioPerfil } = crearFakes({ rolDestino: 'dueño' });
    const caso = new CompartirHistorial(repositorioHistoriales, repositorioMascotas, repositorioPerfil);

    await expect(caso.ejecutar({ datosCrudos: datosValidos, veterinarioOrigenId })).rejects.toBeInstanceOf(VeterinarioNoEncontradoError);
    expect(repositorioHistoriales.crear).not.toHaveBeenCalled();
  });
});
