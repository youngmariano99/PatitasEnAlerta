...
// Mismo criterio que UpstashControlDeTasaAntiSaturacion.test.ts: los mocks se
// definen DENTRO del factory (babel-plugin-jest-hoist mueve jest.mock() por
// encima de cualquier `const` del módulo) y se exponen como `__mockLimit`/
...
const { __mockLimit: mockLimit, __mockSlidingWindow: mockSlidingWindow } = jest.requireMock(
  '@upstash/ratelimit',
) as {
...
describe('UpstashControlDeTasaGeocoding', () => {
  beforeEach(() => {
    mockLimit.mockReset();
  ...
  it('lanza si faltan las variables de entorno de Upstash', () => {
    ...
    expect(() => new UpstashControlDeTasaGeocoding()).toThrow(/UPSTASH_REDIS_REST_URL/);
  });

  it('configura una ventana deslizante de 20 consultas cada 10 minutos', () => {
    // eslint-disable-next-line no-new
    new UpstashControlDeTasaGeocoding();
    expect(mockSlidingWindow).toHaveBeenCalledWith(20, '10 m');
  });

  it('permite la acción cuando Upstash responde success=true', async () => {
    mockLimit.mockResolvedValue({ success: true, reset: Date.now() + 600_000 });
    const adaptador = new UpstashControlDeTasaGeocoding();
  ...
  it('bloquea la acción cuando Upstash responde success=false e informa los segundos hasta poder reintentar', async () => {
    ...
    mockLimit.mockResolvedValue({ success: false, reset: ahora + 45_000 });
    const adaptador = new UpstashControlDeTasaGeocoding();

    const resultado = await adaptador.evaluar('usuario-1');

    expect(resultado.permitido).toBe(false);
    expect(resultado.reintentarEnSegundos).toBe(45);
  });
