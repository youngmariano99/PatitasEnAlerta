import { esCuestionarioCompleto, type DatosCuestionarioAdoptante } from '@dominio/entidades/CuestionarioAdoptante';
...
describe('esCuestionarioCompleto', () => {
  ...
  it.each(['horasSoloEstimadas', 'presenciaNinos', 'espacioDisponible', 'experienciaPrevia'] as const)(
  ...
  it('un cuestionario recién creado (todos los campos null) es incompleto', () => {
    expect(
      esCuestionarioCompleto({ horasSoloEstimadas: null, presenciaNinos: null, espacioDisponible: null, experienciaPrevia: null }),
    ).toBe(false);
  });
});
