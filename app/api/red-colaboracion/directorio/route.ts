...
/**
 * Directorio de aliados verificados (Módulo 5 — Red de Colaboración,
 * Post-MVP): organizacion/ONG, veterinario, rescatista, filtrable por rol y
 * zona. A diferencia de GET /api/reportes, acá sí exige sesión — el prefijo
 * /api/red-colaboracion está protegido en middleware.ts sin excepción de
 * lectura pública. La autorización por rol del solicitante se resuelve en
 * ListarDirectorioAliados.autorizar() (PEA-SIS-002), nunca acá.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    const casoDeUso = container.resolve(ListarDirectorioAliados);
    ...
    logger.error({ err: error }, 'Error no controlado en GET /api/red-colaboracion/directorio');
