  ...
  z
    .object({
      ...
      presenciaNinos: z
        ...
        .transform((valor) => valor ?? null),
      espacioDisponible: z
        .enum(ESPACIOS_DISPONIBLES_SOPORTADOS, {
          invalid_type_error: 'Elegí un espacio disponible válido.',
        })
        .nullable()
        .optional()
        .transform((valor) => valor ?? null),
...
export type DatosCompletarCuestionarioAdoptanteDto = z.infer<
  typeof CompletarCuestionarioAdoptanteSchema
>;
...
registroOpenApi.registerPath({
  ...
  summary:
    'Crea o actualiza el cuestionario de estilo de vida propio del usuario autenticado (Módulo 9, Paso 1).',
  request: {
    body: { content: { 'application/json': { schema: CompletarCuestionarioAdoptanteSchema } } },
  },
  responses: {
    201: {
      description: 'Cuestionario guardado (creado o actualizado).',
      content: { 'application/json': { schema: CuestionarioAdoptanteDtoSchema } },
    },
    400: {
      description:
        'Payload inválido, incluye `espacioDisponible` fuera del catálogo (PEA-SIS-005).',
...
registroOpenApi.registerPath({
  ...
  responses: {
    200: {
      description: 'Cuestionario propio.',
      content: { 'application/json': { schema: CuestionarioAdoptanteDtoSchema } },
    },
    401: {
      description: 'No hay sesión activa (PEA-SIS-001).',
