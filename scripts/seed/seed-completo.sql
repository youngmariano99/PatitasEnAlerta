...
-- 2. Usuarios: Dueños de mascota (1000 — simula la base de vecinos de una
-- ciudad real usando la plataforma)
...
-- 10. Reportes (1798 aleatorios + 2 garantizados más abajo = 1800 en total)
...
-- ver docs/SCHEMA.md) completos solo en un SUBCONJUNTO de las 200 fichas
...
FROM (SELECT id FROM tmp_veterinarios ORDER BY random() LIMIT 20) v1,
     (SELECT id FROM tmp_veterinarios ORDER BY random() LIMIT 20) v2
WHERE v1.id <> v2.id
LIMIT 40;
