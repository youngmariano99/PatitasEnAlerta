...
/**
 * Solicitudes de asistencia veterinaria abiertas, filtrables por zona
 * (Módulo 5 — Red de Colaboración, Post-MVP). Exclusivo de rol
 * `veterinario` — verificado en ListarSolicitudesVeterinarias.autorizar()
...
export async function GET(request: NextRequest) {
  ...
  try {
    const casoDeUso = container.resolve(ListarSolicitudesVeterinarias);
    ...
    logger.error({ err: error }, 'Error no controlado en GET /api/red-colaboracion/solicitudes/veterinaria');
