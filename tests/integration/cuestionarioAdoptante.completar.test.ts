...
import { GET as obtenerCuestionario, POST as completarCuestionario } from '@app/api/adopcion-compatibilidad/cuestionario/route';
...
class RepositorioCuestionariosFalso implements IRepositorioCuestionariosAdoptante {
  ...
  async crear(usuarioIdRecibido: string, datos: DatosCuestionarioAdoptante): Promise<CuestionarioAdoptante> {
    this.contador += 1;
  ...
  async actualizar(id: string, datos: DatosCuestionarioAdoptante): Promise<CuestionarioAdoptante> {
    ...
    const actualizado: CuestionarioAdoptante = { ...existente, ...datos, updatedAt: new Date('2026-09-15T11:00:00.000Z') };
...
describe('Cuestionario de estilo de vida del adoptante (Módulo 9, Paso 1/2/4)', () => {
  ...
  it('AC / Paso 4: completa el cuestionario y lo persiste asociado al usuario autenticado', async () => {
    ...
    expect(repositorioCuestionarios.porUsuario.get(usuarioId)?.experienciaPrevia).toBe('Tuvo un gato durante 5 años.');
  });

  it('completar dos veces actualiza el mismo cuestionario en vez de duplicarlo', async () => {
    autenticarComo(usuarioId);
    await completarCuestionario(crearRequest('/api/adopcion-compatibilidad/cuestionario', 'POST', datosCompletos));

    const segundaRespuesta = await completarCuestionario(
      crearRequest('/api/adopcion-compatibilidad/cuestionario', 'POST', { espacioDisponible: 'casa_patio_grande' }),
    );
  ...
  it('AC: rechaza con 400 un espacioDisponible fuera del catálogo soportado', async () => {
    ...
    const respuesta = await completarCuestionario(
      crearRequest('/api/adopcion-compatibilidad/cuestionario', 'POST', { espacioDisponible: 'mansión' }),
    );
  ...
  describe('GET /api/adopcion-compatibilidad/cuestionario', () => {
    it('AC: devuelve exclusivamente el cuestionario del usuario autenticado, nunca el de otro adoptante', async () => {
      ...
      await completarCuestionario(
        crearRequest('/api/adopcion-compatibilidad/cuestionario', 'POST', { espacioDisponible: 'casa_patio_pequeño' }),
      );

      const respuestaOtro = await obtenerCuestionario(crearRequest('/api/adopcion-compatibilidad/cuestionario', 'GET'));
    ...
    it('responde 404 / PEA-ADOP-003 si el usuario todavía no completó ningún cuestionario', async () => {
      ...
      const respuesta = await obtenerCuestionario(crearRequest('/api/adopcion-compatibilidad/cuestionario', 'GET'));
