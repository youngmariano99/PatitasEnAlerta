...
import { PawPrint, MapPin, CalendarDays } from 'lucide-react';
...
const FILTROS: Array<{ valor: Filtro; etiqueta: string }> = [
  ...
  { valor: 'encontrados', etiqueta: 'Encontrados' },
  { valor: 'operativos', etiqueta: 'Operativos' },
];
 ...
 * operativos municipales en un único mapa, con filtros. No reemplaza a
 * `MapaReportes.tsx` (sigue sirviendo `/reportes`, solo reportes, sin
 * filtros) ni a `MapaEventos.tsx` (sigue sirviendo `/municipio/eventos`) —
 * reutiliza los mismos íconos Flyweight que ambos, sin duplicar esa lógica.
 * El filtrado es 100% client-side sobre los arrays ya fetcheados por la
 * home — no dispara llamadas nuevas.
 */
  ...
  return (
    <div>
      ...
      <div className="overflow-hidden rounded-md border border-surface2">
        <MapContainer center={centro} zoom={13} className="h-72 w-full sm:h-96 lg:h-[480px]">
          <TileLayer
