...
import {
  ...
  type IEstrategiaCompatibilidad,
} from '@dominio/estrategias/EstrategiaCompatibilidad';
...
class RepositorioSugerenciasFalso implements IRepositorioSugerenciasCompatibilidad {
  ...
  async crear(
    datos: Parameters<IRepositorioSugerenciasCompatibilidad['crear']>[0],
  ): Promise<SugerenciaCompatibilidad> {
    this.contador += 1;
    const sugerencia: SugerenciaCompatibilidad = {
      id: `sugerencia-${this.contador}`,
      generadoEn: new Date('2026-09-15T11:00:00.000Z'),
      ...datos,
    };
...
function crearRequest(): NextRequest {
  return new NextRequest('http://localhost/api/adopcion-compatibilidad/sugerencias', {
    method: 'POST',
  });
}
...
describe('POST /api/adopcion-compatibilidad/sugerencias (Módulo 9, Paso 2/4)', () => {
  ...
  beforeEach(() => {
    ...
    container.registerInstance<IRepositorioSugerenciasCompatibilidad>(
      'IRepositorioSugerenciasCompatibilidad',
    ...
    container.registerInstance<IEstrategiaCompatibilidad>(
      'IEstrategiaCompatibilidad',
      new CompatibilidadPorReglas(),
    );
  });
  ...
  it('responde 400 / PEA-ADOP-001 si el cuestionario propio está incompleto', async () => {
    autenticarComo(usuarioId);
    repositorioCuestionarios.cuestionario = {
      ...repositorioCuestionarios.cuestionario!,
