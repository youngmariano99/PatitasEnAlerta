...
/** Pedidos propios del dueño autenticado (`dueño CR(p)`, docs/ROLES.md Módulo 6). */
export async function GET(request: NextRequest) {
  ...
  try {
    const casoDeUso = container.resolve(ListarMisPedidos);
    const resultado = await casoDeUso.ejecutar({
      compradorId: usuarioAutenticado.id,
      pagina,
      porPagina,
    });
    ...
    logger.error(
      { err: error },
      'Error no controlado en GET /api/veterinarios/productos/mis-pedidos',
    );
